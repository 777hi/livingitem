...
- [活物品 Tooltip 系统设计](#活物品-tooltip-系统设计)
  ...
  - [3. 数据链路：服务端 → 客户端](#3-数据链路服务端--客户端)
    ...
    - [3.3 客户端：双缓存模型（通用 + 玩家背包）](#33-客户端双缓存模型通用--玩家背包)
  ...
  - [5. 相位圆盘与展开条（v19.2）](#5-相位圆盘与展开条v192)
    - [5.1 为什么换掉波形](#51-为什么换掉波形)
    - [5.2 相位圆盘（左半区）](#52-相位圆盘左半区)
    - [5.3 相位展开条（下半区）](#53-相位展开条下半区)
    - [5.4 其它读数](#54-其它读数)
  - [6. 量化降脏](#6-量化降脏)
  - [7. 设计原则与坑清单](#7-设计原则与坑清单)
  - [关键文件](#关键文件)
  - [测试覆盖](#测试覆盖)
  - [8. 已知行为：虚假周期域在 tooltip 中的显示](#8-已知行为虚假周期域在-tooltip-中的显示)
    - [8.1 原因](#81-原因)
    - [8.2 对玩家的影响](#82-对玩家的影响)
    - [8.3 建议](#83-建议)
...
- 公式展示字段（非 mFE 定点）：`effDeltaSumPermille`（Σ√|Δᵢ| × 1000，int）——F3+H 高级模式下的发电量公式展示行用；`coilForm`（int，0=铜块 1=雕文 2=切制 3=格栅）——决定形态功能说明行和形态标题的翻译键。
...
n 再大就退化为「看分布形态」，精确计数交给数字读数 `Δφ`（最小相位间隔，见 §5.4）兜底。
...
- 面板底部文字行：
  - 左侧：`P=4t  n=4  Δφ=1t`——其中 **Δφ**（最小相位间隔）为最佳域内各路相位偏移量按升序排列后，相邻偏移的最小差值。例如偏移量 `[0, 1, 2, 3]` 的相邻差 `1,1,1` → Δφ=1t，表示 4 路相位均匀分布在 4t 周期内，每 tick 一路；偏移量 `[0, 0, 2, 3]` 的相邻差 `0,2,1` → Δφ=0t，表示多路相位重叠在同一 tick。Δφ 越大说明相位在周期内散得越开，时间利用率越高；Δφ=0 提示存在同相风险（辐条在圆盘上也会真实贴合提示）。
  - 右侧：`锈级 4/4`——当前有出力的锈蚀等级数。分母固定为 4（未锈/暴露/风化/氧化），分子为活跃锈级数（基础出力 > 0 的等级数），用于快速判断共振的活跃锈级数 N。
- 形态功能说明行（文本层，非高级模式常显）由 `coilForm` 字段决定翻译键：`coilForm=0` → 普通铜块（"基础线圈，全向 4 边感应，适合宽带和简单电路"），`coilForm=1` → 雕文（"移相器，定向感应，可级联移相链"），`coilForm=2` → 切制（"裂相器，读取下降沿，补齐反相相位"），`coilForm=3` → 格栅（"相位加法器，多路同周期信号求和"）。
...
## 8. 已知行为：虚假周期域在 tooltip 中的显示

涂蜡发电机初次接触信号或信号中断后恢复时，tooltip 的「检测周期」字段可能显示**成百上千个不同的周期值**，在若干秒后逐步收缩为正确的周期。这是电力层信号检测机制的正常行为，非 tooltip 系统 bug。

### 8.1 原因

详见 [living-power-tech.md §3.2 虚假周期误读](../tech/living-power-tech.md#虚假周期误读的产生与自愈ema-收敛路径)，简要概括：

1. `SignalTracker` 以 EMA（α=0.5）估计周期，信号中断→恢复时旧 `lastRisingTick` 导致巨大间隔 → `periodTicks` 跃迁到很大值 → 指数衰减收敛回真实值；
2. 收敛路径上的每个中间值（如 502→253→129→66→35→20→12→8→6→5→4）都创建一个独立的 `PhaseDomain`，显示在 tooltip 中；
3. BFS 遍历铜块网络时，每条边有独立的 `SignalTracker`，N 个铜块 × 4 方向 = 最多 4N 个跟踪器同步收敛，放大虚假域数量。

### 8.2 对玩家的影响

| 影响 | 说明 |
|---|---|
| **发电不受影响** | 跳变门控只选「本 tick 有跳变」的最佳域入账，虚假域不被选中 |
| **显示自愈** | `tickCleanup` 在 `max(32, 2×P)` 夹 [32,1200] tick 后将无事件的虚假域自动移除 |
| **稳态钉死** | 收敛后只有一个正确周期域，量化降脏使读数稳定 |

### 8.3 建议

- 初次搭建电路后，等待约 2~3 秒（32 tick）让虚假域自动清理完毕；
- 若信号中断后重建，同样等待清理周期；
- 超大规模铜块网络（>20 块）的虚假域数量正比于网络规模，清理时间不变（仍为 32~1200 tick）。

## 相位圆盘渲染（`LivingWaxedCopperTooltipRenderer`）像素对齐规约

> **本节内容仅在此处有全文**（2026-09-16 从项目记忆迁入）。改这个渲染器
> （或任何自己走点的圆形图元）时，**三条规约必须同时成立**，否则整圈会「偏心」：

1. **点块以 (px, py) 为几何中心** —— `fill(px-r, py-r, px+r+1, py+r+1)`。
   写成 `-r+1` 会让 r=1 退化成 2×2、块心偏 (0.5,0.5)，顶部变 r−0.5、底部变 r+0.5。
2. **步数取偶数** —— `2*ceil(π*r*turns)`，让 i=0 / steps⁄4 / steps⁄2 / 3·steps⁄4
   精确命中 12/3/6/9 点。`ceil(2πr)` 在 r=36 时是 227（奇数）⇒ 6 点落 i=113.5
   永远采样不到 —— **这条对视觉的影响比第 1 条更大**。
3. **用 `symRound` 而非 `Math.round`** —— 后者对 .5 朝 +∞（round(3.5)=4 / round(-3.5)=-3），
...
- **展开条柱位用整数槽宽**：`slotW=max(1, STEM_W/period)`、`barW=slotW-1`、
  ...
  浮点 `round(k*slotW)` 会在 .5 累积 +1，柱间裂出 2px 双缝。
- **φ=0 标记必须在 `renderWheel` 末尾画**：它压在周期环 (r=31)，满相时
  `SPOKE_RING_FULL_COLOR` 回填 r=29..30，先画会被亮青环吃掉底行。
- **环厚只能取奇数（1/3/5…）**：`drawArc` 点块以 (px,py) 为中心向两侧各扩 `dotRadius`，
  要关于整数半径对称就必须奇数宽 —— **2 px 对称环在整数像素网格上不存在**。
  想变细就把 dotRadius 降一档（1→0）；守卫是 `dotRadius < 0` 所以 0 合法。
- **布局留白**（改尺寸常数时一并复核）：`LEVEL_BASE_X=167` 必须等于 `STEM_X+STEM_W=167`
  （否则锈级条内缩 —— `fill` 右端排他，最右像素 = 基线−1）；
  `PANEL_H=115`、`labelY=PANEL_H-12`。当前四边留白 上2/下3/左3/右4。
- **验证工具** `tools/_sim_tooltip.py`：离线复现全部像素图元（`--old` 复现修复前行为、
  `--oldpalette` 复现旧配色）。改渲染前先跑它量化 before/after，比反复启动游戏截图快得多。
  判对称要**对比四个基点方向的径向范围**，别用象限像素计数（会把 x==cx 中心列误判到右侧）。
  检查顺序别漏：**圆盘 → 展开条 → 锈级条 → 底部文字行**（后两块最易漏）。
