...
活物品的 tooltip 分为**文本层**（各功能类 `addToTooltip` 输出的文字读数）与**图形层**（F3+H 高级模式下的仪器面板，如涂蜡发电机的波形示波器与锈级柱状图）。两层的数据都来自同一条展示链路：
...
- 锈级柱状图（`LivingWaxedCopperTooltipRenderer` 右半区）数据源 = `getDisplayEmaByOxidationMilliFe()`（窗口均值 mFE）——柱高相对值，单位不敏感，但切 mFE 后小功率柱子不再消失；
- 示波器（左半区）的相数/周期来自最佳域快照（`DomainSnapshot`），与窗口均值无关。
...
├── client/render/LivingWaxedCopperTooltipRenderer.java  # 仪器面板绘制（示波器 + 柱状图）
