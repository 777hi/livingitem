...
| # | P | 问题 | 选项 | 倾向 | 结论 |
|---|---|---|---|---|---|
| I1 | P1 | 配置项（开关 / 上限 / 衰减系数） | ⚠️ `Config.java` **已于 2026-09-22 删除**（去原版化清理，从未有过真实配置项）⇒ 要做配置需**新建** | 建议至少加总开关 | *待定* |
...
### 2026-09-20（第 27 轮）—— `A3` 辅助挖掘落地 + 活工具 tooltip

🎉 **`A3` 支线实装完成**（`LivingToolAssist`）—— 背包里**没有记忆**的活工具会给玩家的挖掘"搭把手"：
多把**直接相加、不设上限**，材质门槛放行，时运 / 精准采集按槽位顺序归属，每把出过力的都扣耐久。

**⭐ 最大收获：原设计里标着「唯一技术难点」的 `B1`（Mixin `ServerPlayerGameMode`）整个不需要**

NeoForge 三个官方钩子正好一一对应，**零 mixin**：
`PlayerEvent.BreakSpeed`（加速）/ `PlayerEvent.HarvestCheck`（材质门槛）/ `BlockDropsEvent`（附魔归属 + 耐久）。
"玩家正在挖哪一格"它自己会告诉我们，中断 / 换目标天然被处理 ⇒ `B2`、`B3` 一并消失。

**⚠️ 实装中踩到一个极具迷惑性的坑：加速「服务端生效了但没感觉」**

症状：门槛生效、附魔生效、**唯独速度没变快**；日志还显示 `原速=1.0 → 新速=10.0`（服务端确实改了）。

根因：原版 `ServerPlayerGameMode#tick` 里 `incrementDestroyProgress` 的**返回值被丢弃**
（只用来发破坏裂纹）——**服务端不会自己破坏方块**；真正触发破坏的是客户端算完进度后发来的
`STOP_DESTROY_BLOCK`。**破坏的节奏由客户端控制**，而我们的 handler 用
`player instanceof ServerPlayer` 把客户端 `LocalPlayer` 直接挡在门外了。

⭐ **是用户的一句话点破的**：*"空手挖完石头后石头掉落了，还吃到了精准采集附魔效果，就是没有加速"*
—— **门槛和附魔是服务端判的，速度是客户端控的**，于是"一半好一半坏"。

修法：`isAssistable` 两端都放行，速度计算分端（服务端 FakePlayer 精算 / 客户端本地算
`getDestroySpeed + 效率等级²`）。最终由服务端二次校验（`f1 >= 0.7F`）兜底，客户端算快是安全的。

📌 **教训**：*"一半生效一半不生效"时，先问「这两半各由哪一端判定」* —— 比逐行读代码快得多。

**同时新增：活工具 tooltip**（`LivingToolFunction#addToTooltip`）—— 显示「模式 + 记忆内容」：

```
模式: 辅助玩家挖掘          ← 无记忆
模式: 自主挖掘              ← 有记忆
  挖掘记忆: 前方 3.2 格
  交互记忆: 前方 2.8 格 (限定: 橡木原木)
```

⭐ 记忆是**隐形**的（手持才画射线、容器里还要 F3+B），而 `A3` 之后
**「有没有记忆」直接决定这把工具是哪种模式** —— 不说清楚玩家根本分不清"帮手"和"工人"。

- 实测通过（2026-09-20）：加速 ✅（含多把叠加 `候选=5 | bonus=45`）、门槛 ✅、掉落与附魔归属 ✅、tooltip ✅。
- 下一步候选：`J` 组容器内 GUI 交互 / `L37` 实体攻击 / 模型手感微调。
