...
**两次失败的尝试（重要教训，别再走）**：
「应用手持 transform + 抵消 FIXED + 手算/运行时立正补偿」两个版本（补偿 +100° / +10° /
运行时 `rotationTo`）都让**原版工具平躺** —— 数学推导怎么改都对不上，根因未查明
（怀疑 sodium/iris 等渲染管线 mod 对物品渲染链的介入导致实际应用顺序与源码阅读不一致）。
⇒ 结论：**守约定的物品，旧方案就是对的**；不守约定的（BEWLR）走它们自己的 fixed。
