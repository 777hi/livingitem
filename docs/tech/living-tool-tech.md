...
| ~~`K` 组悬浮渲染~~ | ✅ **已实现**（射线侧 2026-09-19 / **辅助环侧 2026-09-21**），见 §11.5、§11.8 |
| ~~联机可见性（待机环）~~ | ✅ **已实现**（2026-09-21），见 §11.9 |
| **联机可见性（挖掘环 / 射线）** | ⏳ 待做 —— 需要额外广播"他正在挖哪一格"（`LivingToolAssistState` 目前是各机器本地记录） |
...
- ⭐ **参考系 = 玩家「水平」朝向 + 世界竖直**（2026-09-21 定稿）——
  **只跟转身，不跟抬头低头**：

  | 基准 | 用途 |
  |---|---|
  | `flat` 水平前方（**只看 yaw**） | 圆平面**法线** ⇒ 圆平面**竖直** |
  | `right = flat × WORLD_UP` | 张成圆平面 |
  | `WORLD_UP` 世界竖直 | 张成圆平面 ⇒ 环内的"上"永远是**世界**的"上" |

- **环心 = 脑袋后面（世界竖直高度 + 水平向后）**：
  `玩家位置 + (0, RING_HEIGHT, 0) − flat × RING_BACK_OFFSET`
  - ⚠️ `RING_HEIGHT = 1.7` 别调低：环半径最大 `1.10`，放胸口（`1.0`）时最低点 `−0.10`，
    **下半环埋进地底**（用户实测）。头部高度则最低点 `0.6`，安全。
  - ⚠️ `RING_BACK_OFFSET = 0.75` 别调小：`0.45` 时某些视角下活工具会**穿透脑袋、挡视线**。
- **挖掘环仍是世界参考系**（法线指向玩家、环内的"上"取世界竖直）——
  目标方块在**世界**里，不属于玩家。

> 🔴 **关键区分：法线【不能】用含俯仰的视线方向**（2026-09-21 用户实测）
>
> 用了之后环**永远正对相机** ⇒ 看起来像**贴在屏幕上**（怎么转都一样），
> 失去"背在背后的物体"的感觉。用户原话：**"这个环应当显示在玩家背后，而非屏幕上"**。
>
> 代价是抬头低头时会**斜看环** ⇒ 环在屏幕上变成**椭圆** —— 这正是"真物体"的表现。
> 且**透视变形 ≠ 模型在转**：盘上每个模型的姿态没有变，只是投影变了。
>
> 📌 **MC 的一个坑**：玩家的**身体朝向和视线方向是同一个量**（`getViewVector`）。
> 所以"以身体为参考系"**必然**导致"永远正对相机" —— 想要"世界物体感"，
> 就必须**主动丢掉俯仰分量**（取 `(look.x, 0, look.z)`）。

> ⚠️ **历史教训：位置和朝向必须用同一套基准。**
> 2026-09-20 试过一版"跟随俯仰"被打回 —— 但那版是**半吊子**：
> **位置**按世界水平偏移、**只有朝向**跟随俯仰 ⇒ 两者不匹配，看着像环在"仰倒"。
> 同理 2026-09-21 改成"水平 + 世界竖直"时，**位置也一起改**（环心不再随俯仰移动）。
>
> ⚠️ 曾经的 `RING_FOLLOW_VIEW`（0~1）旋钮 —— 在"跟随视角"和"钉死世界北"之间插值 ——
> 是**没抓住参考系**时的产物，已**删除**。参考系只有一个，没有强度。
>
> ⚠️ 视线**正上 / 正下**时，"水平前方"退化为**零向量** ⇒ 归一化出 NaN ⇒
> 整圈工具消失。此时**沿用上一次的有效值**（`lastRingFlat`），
> **不能**退回写死的方向，否则环会"啪"地钉到固定一侧（用户实测）。
- **长轴沿半径朝外** ⇒ 工具**头朝外、柄朝圆心**，呈放射状。
  ⚠️ 前提是模型已**立正**（见下文 `MODEL_UPRIGHT_FIX`）—— 否则贴图那 45° 的倾斜会盖过
  这里设的朝向，看起来"怎么摆都不对"。
- **起始角从正上方起**（`RING_START_ANGLE = π/2`）⇒ 单工具**竖直立于头顶**、双工具一上一下。
  ⚠️ 从 `0`（玩家右手边）起步时，只有一个工具会**横躺在右侧、头朝右**，不像"待命"。
- ⭐ **工具姿态：用 MC 原生 `Axis` 旋转（和射线模式逐字同构）** —— 2026-09-21 定稿

  三根轴（参考系 = **圆平面**，不是世界）：

  | 局部轴 | 指向 | 含义 |
  |---|---|---|
  | `+Y` | **径向** `r̂` | 柄（长轴）⇒ **头朝外、柄朝圆心**，延长线过圆心 = **放射状** |
  | `+X` | **圆平面法线**（恒定，朝**前**） | 斧刃 / 鼓出面 ⇒ **⊥ 圆平面**、整圈朝同一方向 ✅ |
  | `+Z` | **切向** `t̂` | 板面法线 ⇒ 板面 **⊥ 圆平面** |

  实现 = **三步 `Axis` 旋转**（`drawModel` 的环分支）：

  ```
  ① mulPose(YP, yaw   = atan2(dir.x, dir.z))
  ② mulPose(XP, pitch = atan2(h, dir.y))      // 把 +Y 转到 dir（径向）
  ③ mulPose(YP, ringRoll)                      // 让 +X 朝前（⊥ 圆平面）
  ```

  `ringRoll` **只依赖 `θ`（第几把）** ⇒ **纯刚性、与视角无关**：
  `yaw`/`pitch` 之后局部 `+X` 落在 `sign(cosθ)·flat`，目标是 `+flat`
  ⇒ `cosθ > 0` 那半圈再转 180° 抵消符号。

  ⚠️ 正上 / 正下两把压在分界线上，会**各自镜像**（用户已确认"没关系"）。

- 🔴🔴 **血的教训：不要自造矩阵去定向模型**（2026-09-21，排查了整整一天）

  最初用的是**自造的正交基矩阵** `new Matrix4f(faceX, dir, faceZ)`。症状：

  | 症状 | 看起来像 |
  |---|---|
  | **柄不指向圆心** | "对齐错了" |
  | **模型姿态随视角乱转**（像每个模型自己在转，而非相对环面固定） | "参考系 / 跟随没做对" |

  ⇒ **症状伪装成"参考系问题"，极难定位** —— 为此把参考系、朝向、布局全改了一遍，全都无效。
  **换成 MC 原生的 `Axis` 旋转后立刻正常。**

  > 📌 **为什么**：`Axis.YP/XP` 用的是 **MC 自己的约定**（手性、坐标系、与 display
  > transform 的配合），而且**和射线模式走的是同一条代码路径**（那条路一直是对的）。
  > 自造矩阵只要有一处不符，整块基就废了。
  >
  > ✅ **通用判据**：某处"对齐"出问题时，**先把它换成与已工作的那条路径逐字同构的实现**，
  > 而不是继续调参数。

- ⚠️ **"⊥ 圆平面"有两个解，只能实测定**：
  `+flat`（**朝前**，背离玩家/相机）或 `−flat`（朝相机）。
  首次实现取了 `−flat` ⇒ 斧刃**朝后**了；改 `+flat` 才对（2026-09-21 用户实测）。

- ⚠️ **不要对滚转角做「按世界方向统一符号」的补救**
  （曾用 `faceZ.y` 当判据翻转 `faceX`，结果斧刃变成"半圈朝前、半圈朝后" = 用户报的**螺旋感**）。
  正确做法：让 `ringRoll` **只由 `θ` 决定** ⇒ 整圈自然刚性。

> 📌 **已废弃并删除的方案（别再走）**
>
> 曾实现过一整套"让图案活起来 / 四方向对称"的机制：**随时间漂移**（`RING_PSI_*`）、
> **立体程度呼吸**（`RING_ALPHA_*`）、**随视角反向补偿**（`RING_PSI_VIEW_WEIGHT`）。
> 它们各自都"有效"，但叠加后环变得很难理解，最终**全部关闭并删除**。
>
> **用户的最终结论**：*"这悬浮工具主要是图个好看。"*
> —— 所以判定标准是**肉眼好不好看**，不是"数学上是否一致"。
> （**符号统一**曾被保留过一段时间，但 2026-09-21 查明它正是「螺旋感」的来源，已移除。）
>
> ⚠️ 重启用前先想清楚：这些机制会让"环的样子"变得难以预测，
> 而调参成本（每一次都要重新感受）远高于收益。
...
| 路径 | 滚转怎么定 | 值 |
|---|---|---|
| **射线上** | 固定补角 | `RAY_ROLL_FIX = +270°` |
| **辅助环** | 按 `θ` 算（让斧刃 ⊥ 圆平面且**朝前**） | `ringRoll`（见上文） |
...
| 项 | 结论 |
|---|---|
| 生效范围 | 仅**玩家背包**；手持普通工具时**也帮**；旁观**跳过**（创造 2026-09-25 放开） |
| 距离 | **不需要**（只作用于玩家当前目标，天然受限） |
| 叠加 | 速度**直接相加、不设上限** —— 用户口径：*"玩家背包槽位数量就已经是上限了"* |
| 材质门槛 | 任意一把活工具挖得动即可 |
| 排序 | 槽位号升序（与 `SlotEntry.slotIndex` 一致） |

#### 多把活工具的「优先级」规则（2026-09-25 确认，仅此处按背包顺序）

| 环节 | 规则 |
|---|---|
| 挖掘速度 | 全部**相加**，无顺序 |
| 材质门槛 | 任意一把满足即放行，无顺序 |
| **掉落 + 经验归属** | ⭐ **唯一按背包顺序的**：`helpers.get(0)`（槽位最靠前那把）重算掉落与经验 —— 即「时运 III 的活镐」和「无时运的活铲」同时在包时，**掉落按靠前那把的附魔算**（即使挖得快的是另一把）。玩家可控（把好附魔的放前面 = 玩法）；曾讨论「按附魔最优」口径，未采纳 |
| 耐久 | 每把出过力的各扣 1 点，无顺序 |

**模组特性（范围挖掘之类）的边界**：辅助挖掘只「加速 + 归属」，**破坏动作仍是玩家自己** ⇒
范围挖掘等特性按玩家主手正常触发，活工具不接管也不干扰。
自主回放（有记忆）则由 FakePlayer 出手 —— 特性能否触发**取决于该模组的判定方式**：
按 UUID 判「玩家」的会触发（FakePlayer UUID = 主人），按实体类型 / 主手物品判的不会。
...
### 11.8 已实现：辅助环（2026-09-21）

无记忆的活工具（"帮手"）在玩家背后围成一圈；挖方块时**能对那个方块出力的**会瞬现到方块处转圈。

**两个环可以同时存在**（按历史版本恢复的分组逻辑）：

| 批次 | 判据 | 环心 | 法线 | 动画 |
|---|---|---|---|---|
| **出力** | `stack.getDestroySpeed(目标方块) > 1.0F` | 目标方块中心 | 指向玩家 | 风车自转（挖得越快转得越快） |
| **其余** | 否则 | 玩家脑袋后面 | 水平前方 | 静止 |

⚠️ **"确实在挖"的判据必须用 `mc.gameMode.isDestroying()`，不能用 BreakSpeed 事件** ——
后者【只要准心对着方块就每 tick 触发】（不需要按住左键）⇒ 会导致"**指着哪就在哪转圈**"
（2026-09-20 用户报过的 bug）。

**几何（用户 2026-09-21 规格）**：柄在圆平面内、延长线过圆心（**放射状**）；斧刃 ⊥ 圆平面且**朝前**。

**参考系 = 玩家「水平」朝向 + 世界竖直**（只跟转身，不跟抬头低头）：

| 基准 | 用途 |
|---|---|
| `flat` 水平前方（只看 yaw） | 圆平面**法线** ⇒ 圆平面竖直 |
| `WORLD_UP` 世界竖直 | 张成圆平面 ⇒ 环内的"上"永远是世界的"上" |

> 🔴 **法线【不能】用含俯仰的视线方向**：那会让环永远正对相机 ⇒ 看起来像**贴在屏幕上**
> （怎么转都一样）。用户原话：*"这个环应当显示在玩家背后，而非屏幕上"*。
> 代价是抬头低头时环会变椭圆（透视）—— **那正是"真物体"的表现，不是模型在转**。
>
> 📌 **MC 的坑**：玩家**身体朝向 = 视线方向**（同一个量）⇒ "以身体为参考系"**必然**导致
> "永远正对相机"。想要世界物体感，必须**主动丢掉俯仰分量**（取 `(look.x, 0, look.z)`）。

> 🔴🔴 **血的教训（排查了整整一天）：不要自造矩阵去定向模型**
>
> 最初用自造的正交基矩阵 `new Matrix4f(faceX, dir, faceZ)` 定向。症状：
> **柄不指向圆心** + **模型姿态随视角乱转**（像每个模型自己在转，而非相对环面固定）。
> **症状伪装成"参考系问题"，极难定位** —— 为此把参考系、朝向、布局全改了一遍都无效。
> **换成 MC 原生的 `Axis` 旋转后立刻正常。**
>
> ⇒ **通用判据**：某处"对齐"出问题时，**先把它换成与已工作的那条路径逐字同构的实现**
> （这里是 `renderOne` 的 `yaw`/`pitch`），而不是继续调参数。

**实现 = 三步 `Axis` 旋转**（`drawModel` 的环分支，与射线模式同构）：

```
① mulPose(YP, yaw   = atan2(dir.x, dir.z))
② mulPose(XP, pitch = atan2(h, dir.y))      // 把 +Y 转到 dir（径向）
③ mulPose(YP, ringRoll)                      // 让 +X（斧刃）朝前（⊥ 圆平面）
```

`ringRoll` **只依赖 θ（第几把）** ⇒ 纯刚性、与视角无关：
`yaw`/`pitch` 之后局部 `+X` 落在 `sign(cosθ)·flat`，目标是 `+flat` ⇒ `cosθ > 0` 那半圈再转 180°。

⚠️ 正上 / 正下两把压在分界线上，会**各自镜像**（用户已确认"没关系"）。

**关键常量**：`RING_HEIGHT = 1.7`（别调低 —— 半径最大 1.10，放胸口会埋进地底）、
`RING_BACK_OFFSET = 0.75`（别调小 —— 0.45 时工具会穿透脑袋）、
`RING_START_ANGLE = π/2`（从正上方起步）、半径 `clamp(0.28 + 0.055n, …, 1.10)`。

---

### 11.9 已实现：联机可见性 · 最小版（2026-09-21）

**问题**：渲染器是**每个客户端各自跑**的，而"背包"那一路只读 `mc.player.getInventory()`
（自己）⇒ **别人看不到你背后的环**。
...
⭐ **包里只有 UUID + 工具** —— 玩家的**位置 / 朝向走原版实体同步**，
客户端渲染时直接读 `Player`（带帧间插值）⇒ **不需要进包**。

⭐ **自己那份不发** —— 本机渲染更实时，而且只有自己能走挖掘环。

⚠️ `RADIUS` **必须与 `LivingToolHostSync.RADIUS` 一致**（两者都是"活工具可视范围"），
否则会出现"**看得到环、看不到射线**"。

⚠️ **挖掘环 / 射线不在最小版范围内** —— 那需要"他正在挖哪一格"，
而 `LivingToolAssistState` 是各机器**本地**记录的（要额外广播）。
