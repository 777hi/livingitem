...
⭐ **它写在 `rollRad` 的外侧，于是两条路径自动各自正确、不用分叉**：

| 路径 | `rollRad` | 自转轴（跟着 rollRad 滚） |
|---|---|---|
| 环上 | `0` | 局部 `Z` = **板面法线** ✅ |
| 射线上 | `+90°` | 局部 `X`（= 改动前的原行为）✅ |

> ⚠️ 位置不能乱放：写在 `rollRad` **内侧**的话，自转轴就固定死在 `Z`，射线上会跟着一起变。
...
- **环平面法线 = 玩家【完整视线方向】**（含俯仰）⇒ <b>抬头低头环也跟着晃</b>（2026-09-20 用户改；
  此前只取水平分量、刻意"抬头低头不晃"）。

  ⚠️ 改这一处牵出两个必须配套处理的点：

  1. **环心仍按【水平】背向偏移** —— 若环心也跟着俯仰走，抬头时环会**掉到脚底**、低头时顶到头顶。
  2. **环平面内的正交基必须重新构造** —— 原先直接借世界 `up` 当第二根轴，那是**以"法线恒为水平"为前提**的；
     法线一倾斜就不再正交。且正对上下时 `normal × up` 会**退化**（叉积为零、归一化出 NaN），
     需用世界 `Z` 兜底。

  挖掘环暂未跟随俯仰（法线仍指向玩家的**水平**方向，环面朝着玩家）。
...
写入（`BreakSpeed` 每 tick 触发，所以"最近有没有收到事件"本身就表达了"在不在挖、挖哪一格"），
**不需要任何网络同步**。静默超过 3 tick 即视为停止。
