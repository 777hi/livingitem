...
- ⭐ **参考系 = 玩家身体**（2026-09-21 用户规格）—— 环心 / 圆平面 / 工具姿态**全部**以身体为准：

  | 身体轴 | 用途 |
  |---|---|
  | `forward` 身体前（**含俯仰**） | 圆平面**法线** ⇒ 圆平面平行玩家背面 |
  | `right` 身体的右 | 张成圆平面 |
  | `bodyUp` 身体的上（**含俯仰**） | 张成圆平面 ⇒ 环内的"上"随身体倾斜 |

- **环心 = 脑袋后面**：`玩家位置 + bodyUp × RING_HEIGHT − forward × RING_BACK_OFFSET`
...
- ⭐ **工具姿态 = 一组正交基，三根轴一次钉死**（不用 `yaw`/`pitch`）
  —— ⚠️ 且**参考系是「圆平面」，不是「世界」**（2026-09-21 用户给出的规格）

  | 局部轴 | 指向（圆平面参考系） | 含义 |
  |---|---|---|
  | `+Y` | **径向** `r̂` | 柄 ⇒ **头朝外、柄朝圆心**（柄在圆平面内、延长线过圆心）|
  | `+Z` | **切向** `t̂ = n̂ × r̂` | 板面法线 ⇒ 板面 **⊥ 圆平面** |
  | `+X` | **圆平面法线** `n̂`（**恒定**） | 斧刃 ⇒ **整圈朝同一方向** ✅ |

  ⚠️ **为什么必须钉死一整组正交基**：`yaw`/`pitch` 只保证「把 `+Y` 转到目标方向」，
  **剩下的滚转自由度是任意的** —— 斧刃会随位置乱翻。环上 `radial` 要绕一圈，
  那个"多余的自由度"必然暴露。

  > 📌 **教训**：**「对齐一根轴」≠「定死朝向」**。
  > 射线只有一个方向自由度，所以射线的 `yaw`/`pitch` 看着没问题；环上不行。

- ⚠️ **不要对 `faceZ` 做「符号统一」**（曾用世界竖直 `faceZ.y` 当判据翻转它）

  因为 `faceX = dir × faceZ`，而 `dir` 到对面**反号** ⇒ **翻转 `faceZ` 会连带翻转 `faceX`**，
  斧刃于是变成"半圈朝前、半圈朝后" —— 正是用户报的**螺旋感**。

  📌 `faceZ` 在对面反向**不是镜像**，而是**绕柄转 180°**。
  **接受它，`faceX` 才自动恒定**（`= ±n̂`），整圈斧刃才统一。

  > 📌 **两次踩坑的共同根因**：**参考系混淆** ——
  > 拿「世界」做参考系，去统一一个本该以「圆平面」为参考系的量。

- ⭐ **朝向跟随 `RING_FOLLOW_VIEW`** —— 环法线是否跟随玩家朝向（**默认 `1`**）

  | 值 | 效果 |
  |---|---|
  | `1`（默认） | 环**始终正对着你**、朝向随身体转 ⇒ 工具看起来是**放射状** ✅ |
  | `0` | 法线钉死在**世界北**，朝向**不随身体转**；环一旦侧对着你，整圈会被**压扁** ⚠️ |

  ⚠️ **它只影响朝向，不影响位置**：环心始终在玩家背后（那里用的是**视角方向**算位置
  —— 位置跟随只是**平移**）。

  📌 **别设成 `0`**（用户实测反馈，两个副作用同时出现）：
  ① 环朝向固定、**不随身体朝向**；
  ② 侧看时整圈工具被**压扁**、不再像放射状。
