...
| K31 | P1 | 动画驱动源 | a) network-only 组件 `LIVING_TOOL_ACTION`（细分 DIG/STRIP/IDLE）<br>b) 客户端自行推断<br>c) 复用原版破坏裂纹档位 | 🔁 **2026-09-19 用户修订 =「忙 / 闲」两态 + c**<br>📌 用户口径：*"客户端只需要知道两点：1. 活工具正在回放记忆 2. 活工具闲着。<br>其它的动画都从客户端自己推断播放。"*<br>→ **不再需要** DIG/STRIP 这类细分，只要 **1 bit 状态**<br>→ `K31=a` 的 `LIVING_TOOL_ACTION` **已不需要**（省掉一个 DataComponent） | ✅ 忙/闲 + c |
