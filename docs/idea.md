...
| F3 | P1 | 挖爆之后 | a) 消失 b) 变损坏物品（是否保留 `IS_LIVING`？） c) 留 1 点耐久不再消耗 | **c** 最友好，但需与 F1 口径自洽 | *待定* |
...
| # | P | 问题 | 选项 | 倾向 | 结论 |
|---|---|---|---|---|---|
| K1 | P0 | **掉落物形态要不要「工作」（tick）？** | a) 只显示模型，不 tick（掉落物 = 休眠态）<br>b) 也 tick（掉落物也能帮挖 / 自己动） | ✅ **b**（2026-09-18 用户拍板「掉落物也工作」）<br>→ 需要 `K.2` 的服务端掉落物 tick 扩展（`ItemEntityContainerContext` 或等价单栈入口） | ✅ b |
| K2 | P0 | **容器形态的 S2C 同步怎么做？** | a) 新增独立包 `LivingFloatSyncPacket`（dim + 位置列表 + 代表 ItemStack）<br>b) 复用/改造 `LivingItemSyncPacket`<br>c) 复用 `ContainerRuntimeCache` | **a**。b/c 是 tooltip 通道，**只发给正在开 GUI 的玩家**（`isViewingContainer`），承担不了世界渲染广播 | *待定* |
...
| K31 | P1 | 动画驱动源 | a) network-only 组件 `LIVING_TOOL_ACTION`<br>b) 客户端自行判断<br>c) 复用原版破坏裂纹档位 | ✅ **a + c 组合**（2026-09-18 用户采纳倾向）<br>a 提供"在做什么"（DIG / STRIP / IDLE），c 提供"挖到第几阶段"的节奏 | ✅ a+c |
...
| L14 | **P0** | **容器 / 掉落物的射线起点在哪？**（它们没有眼睛） | a) 容器方块中心 `pos+0.5`<br>b) 容器顶部 `pos+(0.5,1,0.5)`<br>c) 对齐玩家眼高<br>d) 掉落物：实体位置 | ✅ **a（容器方块中心）** / **d（掉落物实体位置）**（2026-09-18 用户定）<br>📌 **用户口径**：*"录制的结果是一条有长度有方向的射线，原点在玩家哪个部位不重要，重要的是这条线本身"*<br>→ 记忆 = **一条线段**（方向 + 长度），回放 = **把这条线平移到宿主身上**，不追求复现玩家当时的相对高度 | ✅ a/d |
...
| L19 | **P1** | **待机时的目标指示**：让玩家看见活工具打算挖哪 | a) **一条线**<br>b) 高亮目标方块<br>c) 线 + 框<br>d) 工具虚影 / 光晕 | ✅ **a（简单一条线）**（2026-09-18 用户定）<br>📌 **前提澄清**：挖的过程中**不必画** —— 复用原版破坏裂纹（`destroyBlockProgress`）即可。<br>本条只管「待机 / 未开始挖」时，画**宿主 → 命中点**一条线 | ✅ a |
| L20 | P1 | 可视化的**显示时机** | a) 一直显示<br>b) 看着宿主<br>c) 潜行<br>d) 手持同类工具<br>e) 极淡常驻<br>**f) 按 F3+B 时** | ✅ **f（F3+B 显示碰撞箱时才显示）**（2026-09-18 用户定）<br>读取：`mc.getEntityRenderDispatcher().shouldRenderHitBoxes()`<br>纯客户端状态，无需同步（见 `L20 详解`） | ✅ f |
...
| L27 | P0 | ⚠️ **FakePlayer 的 `tick()` 是空实现** —— 破坏进度**不会自动推进** | — | 硬约束：必须在我们自己的 tick 里**手动推进进度**，<br>到 `>= 1` 时再调 `gameMode.destroyBlock(pos)`（或复刻 `tryHarvestBlock`）。<br>这正是 Create 自己维护 `blockBreakingProgress` 的原因 | ✅ 已知 |
