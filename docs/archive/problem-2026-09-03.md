...
- **激进拆分（后续跟进）**：`processContext` 从 170 行降至 ~50 行，拆分为 6 个阶段子方法（`handleEmptyContainer` / `tickFunctionSlots` / `runFunctionTicks` / `flushEnderChannels` / `writebackBlockEntities` / `incrementCleanup`）。`instanceof SimpleContainerContext` 收拢到 2 处。Perf 相位边界不变。编译 + 169 全量测试通过。文档同步。
...
### P2-8（双箱位置重复实现）—— 已完成（2026-09-03）

- 将 `ContainerLivingItemHandler` 与 `CrossContainerTransfer` 中完全相同的 `findDoubleChestPositions` 方法提取到新工具类 `DoubleChestPositions` 中，消除重复。
- `ContainerLivingItemHandler.findDoubleChestPositions` 改为委托 `DoubleChestPositions.find(level, pos)`；`CrossContainerTransfer` 改为直接调用 `DoubleChestPositions.find`。
- 编译 + 全量测试通过。

### P2-10（列数推断两套算法）—— 已完成（2026-09-03）

- 分析确认 `SimpleContainerContext.guessWidth` 是死代码（生产环境从不执行，`findContainerRule()` 总是回退到 `findOrGenerateRule` 返回的规则）。
- `ContainerCompatibilityConfig.resolveColumns(int)` 改用加权启发式：候选宽度 `{9, 10, 12, 13, 8, 7, 6, 11, 5, 4, 3, 2, 1}`，按真实模组分布排序。
- 新增 `resolveColumns(int, Container)` 公开方法，集成三层策略：
  - **Tier 1**：已注册规则（BE 类型匹配）
  - **Tier 2**：已知容器类型 instanceof 推断（Chest→9, Hopper→1, Dispenser→3, Furnace→1, 等）
  - **Tier 3**：加权启发式回退
- `SimpleContainerContext.getWidth()` 重构为三层策略；`findContainerRule()` 不再回退到 `findOrGenerateRule`；删除 `guessWidth` 死代码。
- 测试更新：36 格容器预期从 12 列修正为 9 列（36%9=0，9 权重高于 12）。编译 + 144 全量测试通过。

### P2-11（盔甲槽测试盲区）—— 已完成（2026-09-03）

- 新增 `SimpleContainerContextTest`，覆盖：
  - **基础委托**：`getSize`/`getItem`（含越界 EMPTY）/`getSlotLimit`/`isItemValid`/`getMaxStackSize` 全部委托给 handler
  - **列数推断（Tier 3 加权启发式）**：27→9、36→9、40→10、54→9、96→12、120→10、质数→1、1→1、2→2
  - **写入路径**：空槽写入、越界无害、覆盖替换、堆叠上限截断
  - **模拟插入**：空槽/部分满/满槽/不同物品/无 Container 回退
  - **盔甲槽路径**（P2-11 核心）：`isItemValid` 返回 true、`getSlotLimit` 返回 maxStackSize、`simulateInsertItem` 按 maxStack 截断且忽略已有物品
  - **容器标识**：无 inventory/BE 时使用 handler 哈希
- 使用 Mockito mock `Player` + 真实 `Inventory` 构造模拟盔甲槽场景。
- 全部 25 项测试通过，全量 169 测试无回归。

### P1-5（processContext 激进拆分）—— 已完成（2026-09-03）

- `processContext` 从 170 行降至 **~50 行**（只剩编排调用 + perf 计时），拆分为 6 个清晰命名的子方法：
  - **`handleEmptyContainer`** — 空容器残留红石归零
  - **`tickFunctionSlots`** — 功能槽位集合写入 TickContext
  - **`runFunctionTicks`** — 逐功能执行 tick 逻辑
  - **`flushEnderChannels`** — EnderChannel 脏通道刷新 + 水桶冗余流清理
  - **`writebackBlockEntities`** — 应力/流体写回 BlockEntity（或玩家脚底）
  - **`incrementCleanup`** — 过期数据清理
- `processContext` 代码结构变为六阶段编排，每阶段职责单一、`instanceof SimpleContainerContext` 收拢到 2 处（`handleEmptyContainer` + `writebackBlockEntities`）。
- `PerfMetrics.recordPhase` 边界保持不变（`scan`/`func_tick`/`flush_channels`/`container_data`/`stress`/`flush_slots`），与历史 perf 数据兼容。
- 编译 + 169 全量测试通过。
- 文档同步：`docs/system-design/living-item-infrastructure.md` §8.2 更新为六阶段设计描述。

### P0-3（世界容器同步 `==` 匹配修复）—— 已完成（2026-09-03）

- `syncWorldContainer` 移除 `slot.getItem() == stack` 引用相等检查，替换为 `slot.container != serverPlayer.getInventory()` 归属判断，防止误匹配菜单中的玩家背包槽位。
- 编译 + 169 全量测试通过。