...
所有 jar 位于 `build\moddev\artifacts\` 目录。

## 资源 Jar（resources）

**路径**: `build\moddev\artifacts\neoforge-21.1.230-client-extra-aka-minecraft-resources.jar`

### 列出文件

```powershell
jar tf "build\moddev\artifacts\neoforge-21.1.230-client-extra-aka-minecraft-resources.jar" | Select-String "<关键词>"
```
...
```powershell
# 方块模型
jar tf ... | Select-String "models/block/"

# 物品模型
jar tf ... | Select-String "models/item/"

# 纹理
jar tf ... | Select-String "textures/"

# 方块状态
jar tf ... | Select-String "blockstates/"

# 配方
jar tf ... | Select-String "recipe/"

# 战利品表
jar tf ... | Select-String "loot_table/"

# 标签
jar tf ... | Select-String "tags/"
```

### 提取文件

```powershell
jar xf "build\moddev\artifacts\neoforge-21.1.230-client-extra-aka-minecraft-resources.jar" "assets/minecraft/models/block/comparator.json" "assets/minecraft/blockstates/comparator.json"
```
...
```powershell
# 查找某个类
jar tf "build\moddev\artifacts\neoforge-21.1.230-sources.jar" | Select-String "ComparatorBlock"
```

### 提取源码

```powershell
jar xf "build\moddev\artifacts\neoforge-21.1.230-sources.jar" "net/minecraft/world/level/block/ComparatorBlock.java"
```
