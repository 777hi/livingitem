...
public final class PowerMath {
     ...
     *
     * <p>标定 1,000,000（2026-09-09，1,000 → 10,000 → 1,000,000）：一堆(64) 64M FE，
     * 对齐科技生态中高档单格电池（对标 Mek 能量立方 / Flux 储存器档位），
     * 「充电宝物流」彻底实用（一盏 = 一个满配方块电池）；满溢反馈环保存——
     * 早期产出（个位数 FE/t）下充满一盏需数十小时，反馈环自然退化为「长线目标」，
     * 与「前期小电池、后期堆规模」的玩法节奏一致。q 按 mFE 绝对值存储，扩容对
     * 旧存量灯 = 白赚头寸，无迁移问题。</p>
     *
     * <p><b>int 收窄警戒线（放大后需正视）</b>：对外 IEnergyStorage 是 int FE，
     * 单堆溢出需 count ≥ 2,148 盏（2³¹/1M = 2,147.48）——**超大堆叠容器（抽屉类，上限可达数千）
     * 已可触达**；64 原版堆下安全（一堆 64M &lt; int 上限 21.4 亿）。long 内部
     * 全程无损；int 收窄只影响外部读数与单次请求粒度，fail-safe 不崩不刷
     * （超大堆叠审计 §2.8 结论维持）。</p>
     */
    public static final long BULB_UNIT_CAPACITY_FE = 1_000_000;
    ...
    /**
     * 溢出安全的 {@code floor(a × b / c)} —— 「按剩余容量比例分配」这类份额计算的唯一入口。
     *
     * <p><b>为什么必须走这里</b>（2026-09-11 真凶）：本模组的定点制把量级抬得很高——
     * 每盏铜灯容量 {@link #BULB_UNIT_CAPACITY_MFE} = 1e9，单容器容量可达 1e12，
     * 单机发电量可达 1e6 RE。于是 {@code a * b} 轻易越过 {@code Long.MAX_VALUE}
     * （9.22e18）：{@code 1.7e12 × 6.4e10 = 1.1e23}。</p>
     *
     * <p>溢出后果不是崩溃而是<b>静默变质</b>：份额变负 → 本轮分配量为 0 →
     * 「零头回收」把整个请求量当成零头，每轮只扣几十 mFE，退化成约 10 亿轮
     * （实测 27 槽 × 64 盏 + 外部请求 50 万 FE ⇒ 7.8e6 次取物品 ≈ 1.17s/tick，
     * 与 spark 慢 tick 的 1094ms 吻合）；或份额变成巨大正数 → 分配量被夹到容量上限
     * → 凭空造电。两个调用点（容器充电、发电直存）曾各写一遍同款错误。</p>
     *
     * <p><b>做法</b>：先转 {@code double} 算比例。量级 &lt; 2^53 时与整数精确一致
     * （覆盖全部测试与常规场景），超出时相对误差 ~1e-16；末尾回退一步保证不会因浮点
     * 向上取整而多算 1 —— 宁可少分，不可凭空造。</p>
     *
     * <p>约定 {@code a} 是待分配总量、{@code c} 是分母总量，故调用方通常满足
     * {@code a ≤ c}；若 {@code a > c}（发电量超过容器剩余容量）按 {@code a = c} 处理，
     * 语义等价于「份额不超过 b」——与 {@code Math.min(CAP, ...)} 的后续夹取一致。</p>
     *
     * @param a 待分配总量（≥ 0）
     * @param b 比例因子（≥ 0，也是结果的天然上界）
     * @param c 分母总量（&gt; 0）
     */
    public static long mulDivFloor(long a, long b, long c) {
        if (a <= 0 || b <= 0 || c <= 0) return 0;
        if (a > c) a = c;
        long r = (long) ((double) a * b / c);
        if (r > b) {
            r = b;                                              // 浮点误差兜底
        } else if (r > 0 && (double) r * c > (double) a * b) {
            r--;                                                // 浮点向上多算 1 时回退
        }
        return r;
    }
